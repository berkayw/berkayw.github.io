...
function AddForceAtPosition(force, hitPoint) {
    if (!picker.picked) return;

    
    const modelWorldPosition = new THREE.Vector3();
    picker.picked.getWorldPosition(modelWorldPosition);

    const localHitPoint = hitPoint.clone().sub(modelWorldPosition);
    torqueToApply.add(localHitPoint.cross(force));
    forceToApply.add(force);
}
...
export class ModelPreviewer{
    ...
    Update () {
        ...
        this.portfolioItems.forEach(item => {
            ...
            const baseScale = item.appeared;
            let finalScale = baseScale;
        
            if(item.object.name === "pirate-ship") 
            { 
                finalScale *= 1; //Scale
                item.object.position.set(0,-.5,0)
            }
            else if(item.object.name === "stone_gateway") 
            {
                finalScale *= 1; //Scale
                item.object.position.set(0,-.5,0)

            }
            else if(item.object.name === "toei_sonic_head") 
            {

                finalScale *= .07; //Scale
                item.object.position.set(0,-4.5,0)
            }
            else if(item.object.name === "paper_airplane") 
            {                
                finalScale *= .25; //Scale
                item.object.position.set(0,-2,0);
